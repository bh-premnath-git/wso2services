...
echo "Checking for existing UNIQUE constraint..."
CONSTRAINT_EXISTS=$(docker exec -i "$MYSQL_CONTAINER" mysql -u"$MYSQL_USER" -p"$MYSQL_PASSWORD" -s -N "$MYSQL_DB" <<EOF 2>/dev/null
SELECT COUNT(*)
...
if [ "$CONSTRAINT_EXISTS" -gt 0 ]; then
    echo "✓ UNIQUE constraint already exists. Migration not needed."
    echo ""
...
echo "⚠ UNIQUE constraint NOT found. Migration needed."
echo ""

# Check for duplicate entries
echo "Checking for duplicate entries..."
DUPLICATE_COUNT=$(docker exec -i "$MYSQL_CONTAINER" mysql -u"$MYSQL_USER" -p"$MYSQL_PASSWORD" -s -N "$MYSQL_DB" <<EOF 2>/dev/null
SELECT COUNT(*) - COUNT(DISTINCT CONCAT(UM_RESOURCE_TYPE, '|', UM_RESOURCE_ID, '|', UM_POLICY_HOLDING_ORG_ID))
FROM UM_RESOURCE_SHARING_POLICY;
EOF
)

if [ "$DUPLICATE_COUNT" -gt 0 ]; then
    echo "⚠ Found $DUPLICATE_COUNT duplicate entries that will be removed"
else
...
EOF
    ...
    echo "2. Check logs: docker compose logs wso2is 2>&1 | grep -E 'ERROR|Exception|SEVERE|FATAL'"
    echo ""
else
    echo ""
